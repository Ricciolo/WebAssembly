...
(0, mocha_1.describe)('WapcHost', function () {
    ...
    it('guests should be able to invoke host calls', async () => {
        ...
        const host = await (0, src_1.instantiate)(buffer, (binding, namespace, operation, payload) => {
            expect(namespace).to.equal('myNamespace');
            expect(binding).to.equal('myBinding');
            expect(operation).to.equal('myOperation');
            const decoded = (0, msgpack_1.decode)(payload);
            expect(decoded).to.equal('this is the payload');
            return new Uint8Array((0, msgpack_1.encode)(decoded.toUpperCase()));
        });
        const payload = (0, msgpack_1.encode)({
    ...
    it('should run multiple guests', async () => {
        const host = await (0, src_1.instantiate)(buffer);
        const host2 = await (0, src_1.instantiate)(buffer);
        const result = await host.invoke('echo', (0, msgpack_1.encode)({ msg: 'world' }));
        const decoded = (0, msgpack_1.decode)(result);
        const result2 = await host2.invoke('echo', (0, msgpack_1.encode)({ msg: 'world2' }));
        const decoded2 = (0, msgpack_1.decode)(result2);
    ...
    it('should throw when passed invalid wasm', async () => {
        ...
        await expect((0, src_1.instantiate)(buffer)).to.be.rejectedWith(new src_1.errors.InvalidWasm(new Error()).matcher());
    });
});
